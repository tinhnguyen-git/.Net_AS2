...
{
    public partial class AS2MongoRepository<T> : IAS2Repository<T> where T : BaseEntity
    {
        public AS2MongoRepository(IAS2DatabaseContext dbContext)
        {
            var connectionString = dbContext.GetConnection();
            var client = new MongoClient(connectionString);
            var databaseName = new MongoUrl(connectionString).DatabaseName;
            _database = client.GetDatabase(databaseName);
            _collection = _database.GetCollection<T>(typeof(T).Name);
        }
        public AS2MongoRepository(string connectionString)
        {
            var client = new MongoClient(connectionString);
            var databaseName = new MongoUrl(connectionString).DatabaseName;
            _database = client.GetDatabase(databaseName);
